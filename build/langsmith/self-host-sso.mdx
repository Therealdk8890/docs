...
- You may upgrade a [`basic` auth](/langsmith/self-host-basic-auth) installation to `oauth` mode, but not a [`none` auth](/langsmith/authentication-methods#none) installation.
    - Upgrade a `basic` auth installation by removing the [`basic` auth configuration parameters](/langsmith/self-host-basic-auth#configuration). Users may then login via OAuth **only**. To maintain access post-upgrade, **you must have access to log in via OAuth using an email address that previously logged in via basic auth.**
...
- If you have **not** already configured Oauth with client secret or if you only have personal orgs, you must provide an email address to assign as the initial org admin for the newly provisioned SSO org. If you are upgrading from [basic auth](/langsmith/self-host-basic-auth), LangSmith will reuse your existing org instead.
...
From the app's **Overview** page, record the following values for use in Helm or Docker configuration:
...
Apply the values from the previous steps to your Helm or Docker configuration:
...
By default, LangSmith encodes the spaces between `oauthScopes` in the authorization request as `+` (the `application/x-www-form-urlencoded` convention). Most identity providers accept this, but some (for example, CA SiteMinder) do not decode `+` as a space and reject the login with an `invalid_scope` error. If your IdP rejects multi-scope logins for this reason, set `urlEncodeScopeSpaces: true` (Helm) or `OAUTH_URL_ENCODE_SCOPE_SPACES=true` (Docker) to encode the spaces as `%20` instead, which is the encoding accepted by all OIDC-compliant providers. This applies only to `authType: mixed` with an OAuth client secret, and defaults to off.
...
<Warning>
`POST` and `DELETE` also exist on this resource but are not covered here. `POST` risks creating a duplicate provider row for the org, and `DELETE` removes the SSO provider entirely, which locks out login.
</Warning>
</Tab>
</Tabs>
...
<Warning>
The PKCE workflow is **deprecated** and will be removed in a future release of LangSmith Self-hosted.
</Warning>
