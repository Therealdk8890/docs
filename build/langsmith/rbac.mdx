...
<Info>Role restriction is available for organizations on the Enterprise plan.</Info>

Organization Admins and Organization Operators (roles with `organization:manage`) can mark any role — system or custom — as **restricted**. Restricted roles can only be assigned to workspace members by users who hold the built-in **Workspace Admin** role. Users who have the `workspaces:manage-members` permission but do not hold the Workspace Admin role cannot assign restricted roles.
...
<Note>
Role restriction applies to both system roles (Workspace Admin, Workspace Editor, Workspace Viewer) and custom roles. It is available on LangSmith Cloud and self-hosted deployments.
</Note>
